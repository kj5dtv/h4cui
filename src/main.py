...
from typing import List, Tuple, Optional, Any, Tuple


DEFAULT_HUB4COM_PATH = "C:\\Program Files (x86)\\com0com\\hub4com.exe"
BAUD_RATES: Tuple[int, ...] = (300, 1200, 2400, 4800, 9600, 14400, 19200, 38400, 57600, 115200)
DEFAULT_BAUD = 115200
...
class Hub4ComApp(tk.Tk):
    ...
    def _create_port_frame(self) -> None:
        ...
        self.instructions.grid(row=0, column=0, columnspan=2, sticky='ew', padx=self.internal_padx, pady=self.internal_pady)
        self.port_frame.grid(row=0, column=0, rowspan=3, sticky="ew", padx=self.external_padx, pady=self.external_pady)
    ...
    def _create_physical_ports(self) -> None:
        ...
        _sorted_physical_ports: List[Any] = sorted(
            ...
            key=lambda port: port.port_name
        )
        ...
        first_available_port: Any | None = next(
            (port for port in _sorted_physical_ports if not port.is_in_use), 
            None
        )
    ...
    def _create_cnc_ports(self) -> None:
        ...
        self.check_frame = tk.LabelFrame(master=self.port_frame, text="CNC Ports", padx=self.internal_padx, pady=self.internal_pady)
        self.check_frame.grid(row=1, column=1,sticky="nsew", padx=self.internal_padx, pady=self.internal_pady)
    ...
    def _create_instructions(self) -> None:
        ...
        self.instructions_frame.grid(row=0, column=1, sticky="nsew", padx=self.internal_padx, pady=self.internal_pady)
        ...
        self.instructions_label.grid(row=0, column=0, sticky='ew', padx=self.internal_padx, pady=self.internal_pady)    
            
    def _create_hub4com_path_elements(self) -> None:
        """Creates the hub4com browse to the location of hub4com in the UI thingy."""
        self.hub4com_path_frame = tk.LabelFrame(self, text="Hub4Com Path", padx=self.external_padx, pady=self.external_pady)
        self.hub4com_path_frame.grid(row=1, column=1, columnspan=1, sticky="sew", padx=self.internal_padx, pady=self.internal_pady)
        self.hub4com_path_frame.grid_columnconfigure(0, weight=1)  # Set the weight of the first column to 1
        self.hub4com_path_frame.grid_columnconfigure(1, weight=0)  # Set the weight of the second column to 0
    
        self.hub4com_path_label = tk.Label(self.hub4com_path_frame, text=self.hub4com_path, anchor="nw")
        self.hub4com_path_label.grid(row=0, column=0, padx=self.internal_padx, pady=self.internal_pady, sticky="ew")
    
        self.hub4com_path_button = tk.Button(self.hub4com_path_frame, text="Browse", command=self.browse_hub4com_path, padx=self.internal_padx, pady=self.internal_pady, anchor="ne")
        self.hub4com_path_button.grid(row=0, column=1, padx=self.internal_padx, pady=self.internal_pady, sticky="e")       
    ...
    def _create_baud_rate_dropdown(self) -> None:
        ...
        self.baud_rate_frame.grid(row=2, column=1, columnspan=1, sticky="sew", padx=self.internal_padx, pady=self.internal_pady)
        ...
        self.baud_rate_label.grid(row=0, column=0, sticky='e', padx=self.internal_padx, pady=self.internal_pady)
        ...
        self.baud_rate_dropdown.grid(row=0, column=1, sticky='ew', padx=self.internal_padx, pady=self.internal_pady)
    ...
    def _create_command_label(self) -> None:
        ...
        self.command_label.grid(row=4, column=0, columnspan=2, sticky='ew', padx=self.external_padx, pady=self.external_pady)
    ...
    def _create_status_text(self) -> None:
        ...
        self.status_text.grid(row=5, column=0, columnspan=2, sticky='nsew', padx=self.external_padx, pady=self.external_pady)
        self.status_text.insert(tk.END, "hub4com status will appear here.")
        
    def _create_buttons(self) -> None:
        """Creates the buttons at the bottom of the UI"""
        self.button_frame = tk.Frame(self)
        self.button_frame.grid(row=6, column=0, columnspan=2, sticky="s", padx=self.external_padx, pady=self.external_pady)
        self.run_button = tk.Button(self.button_frame, text="Run", command=self.run_command, padx=self.external_padx, pady=self.external_pady)
        self.run_button.grid(row=0, column=0, padx=self.internal_padx, pady=self.internal_pady)
        self.stop_button = tk.Button(self.button_frame, text="Stop", command=self.stop_command, padx=self.external_padx, pady=self.external_pady)
        self.stop_button.grid(row=0, column=1, padx=self.internal_padx, pady=self.internal_pady)
        self.exit_button = tk.Button(self.button_frame, text="Exit", command=self.exit_command, padx=self.external_padx, pady=self.external_pady)
